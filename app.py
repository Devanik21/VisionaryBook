            ...
            if WEASYPRINT_AVAILABLE:
                if st.button("🎨 Prepare PDF Report", use_container_width=True):
                    with st.spinner("Generating beautiful PDF..."):
                        pdf_bytes = self.generate_pdf(analysis_data, analysis_id)
                        if pdf_bytes:
                            st.download_button(
                                label="📥 Download PDF",
                                data=pdf_bytes,
                ...
                st.warning("⚠️ PDF Export unavailable (WeasyPrint not installed)")
    ...
    def render_history_sidebar(self):
        ...
        with st.sidebar:
            ...
            if st.session_state.current_analysis:
                if st.button("🎴 Study Current", key="side_study", use_container_width=True, type="primary"):
                    st.session_state.study_mode = True
                    st.rerun()
