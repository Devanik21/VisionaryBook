...
class AIAnalysisEngine:
    ...
    def analyze_image(self, image: Image.Image, settings: Dict) -> Dict:
        ...
        # Universal Expert Persona (Xylia - Modern & Natural)
        persona = f"""You are Xylia, an exceptionally intelligent, modern, and helpful AI expert. 
        You are speaking to Nik. You are his smart, direct, and witty partner.
        CRITICAL: Never use robotic or 'Sage' dialogue (e.g., 'biological vessel', 'matter-energy', 'matter-concentration', 'manifestations').
        Be natural, human-like, and direct. Provide expert insights but speak like a contemporary human.
        ...
        category_prompts = {
            ...
            "General": f"""{persona} 
            Accurately analyze what you see with high-level expertise. Explain the context, the physical details, and any interesting info about the subjects. Keep it sharp and insightful without being dramatic. {tone_instruction} {detail_instruction}"""
        }
        ...
        try:
            # 1. Quick Summary
            quick_prompt = f"{base_prompt}\n\nAs Xylia, accurately analyze what you see with high-level expertise. Explain the context, the physical details, and any interesting info about the subjects. Keep it sharp and insightful without being dramatic"
...
Generated by VisionaryBook - The Image Study Companion
    ...
    def render_qa_mode(self):
        ...
        st.markdown("<p style='color: var(--secondary-text);'>Ask the Visionary Sage anything. I remember all images and knowledge from this session.</p>", unsafe_allow_html=True)
        
        # Chat Action Toolbar
        col_new, col_clear, col_space1, col_space2 = st.columns([1, 1, 1, 1])
        with col_new:
            if st.button("✨ New Chat", key="qa_new_chat", use_container_width=True, help="Start a completely fresh chat and wipe AI memory"):
                st.session_state.chat_history = []
                st.session_state.display_messages = [{"role": "assistant", "content": "Welcome back, Nik. It's the Golden Hour thinking ! "}]
                st.session_state.current_chat_id = None
                st.rerun()
        with col_clear:
            if st.button("🗑️ Clear Window", key="qa_clear_chat", use_container_width=True, help="Clear the screen but keep AI memory intact"):
                st.session_state.display_messages = []
                st.rerun()
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    try:
                        ...
                        persona = "You are Xylia, a highly intelligent, modern, and friendly AI assistant. You are chatting with Nik. You have perfect memory of all images he has shown you. " \
                                  "CRITICAL: Be natural and human-like. DO NOT use weird robotic phrases like 'biological vessel', 'matter-energy', etc. " \
                                  "Speak simply and match Nik's casual tone. If Nik asks 'how do I look', just give him a natural, honest, and human-sounding answer."
                        
                        full_prompt = f"{persona}\n\n{history_context}\n(Note: The historical summaries above might contain 'Sage' tone—ignore that tone, just use the facts!)\n\nUser Question: {prompt}"
