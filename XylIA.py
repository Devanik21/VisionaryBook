...
class AIAnalysisEngine:
    ...
    def setup_gemini(self):
        """Setup Gemini API using st.secrets with Multi-Tier Model Architecture"""
        if "GEMINI_API_KEY" in st.secrets and st.secrets["GEMINI_API_KEY"]:
            try:
                genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
                
                # Tier 1: The Base (Open Weights) - Deep, transparent textual reasoning
                self.model_base = genai.GenerativeModel('gemma-3-27b-it')
                
                # Tier 2: The Lite - High-speed, low-latency tasks
                self.model_lite = genai.GenerativeModel('gemini-flash-lite-latest')
                
                # Tier 3: The Main - Multimodal powerhouse
                self.model_main = genai.GenerativeModel('gemini-flash-latest')
                
            except Exception as e:
                st.error(f"Error configuring Gemini API models: {e}")
                self.model_base = None
                self.model_lite = None
                self.model_main = None
        else:
    ...
    def _extract_tags(self, image: Image.Image, summary: str, details: str, category: str) -> List[str]:
        """Extract intelligent tags using the AI model (Lite Tier)"""
        if not self.model_lite:
        ...
        try:
            ...
            # Use Lite Tier for faster processing (no image context needed for tags)
            tag_config = {"temperature": 0.2, "max_output_tokens": 100}
            response = self.model_lite.generate_content(tag_prompt, generation_config=tag_config)
...
class FlashcardManager:
    ...
    def generate_flashcards(self, analysis_data: Dict, analysis_id: str) -> List[str]:
        """Generate high-quality flashcards using AI (Lite Tier)"""
        flashcards = []
        
        if not self.ai_engine.model_lite:
        ...
        try:
            ...
            # Use Lite Tier for high-speed structural generation
            generation_config = {"temperature": 0.2, "max_output_tokens": 800}
            response = self.ai_engine.model_lite.generate_content(qa_prompt, generation_config=generation_config)
            ...
            if st.session_state.socratic_question:
                ...
                if user_hypothesis and st.button("Submit Hypothesis", key="socratic_submit_btn"):
                    if self.ai_engine.model_base:
                        with st.spinner("Xylia is evaluating your deduction using the Base Engine..."):
...
Evaluate Nik's answer. Be encouraging but honest. If he's close, praise his observation skills and fill in what he missed. If he's off, gently redirect him toward the truth. Keep it conversational and engaging. End with the full correct explanation."""
                            ...
                            try:
                                # Text reasoning -> Base Tier (Open Weights)
                                eval_response = self.ai_engine.model_base.generate_content(
                                    eval_prompt,
        ...
        with st.expander("Layer 4: Frontier Discovery Engine", expanded=False):
            ...
            if st.button("Enter Discovery Mode", key="discovery_btn", use_container_width=True):
                ...
                if self.ai_engine.model_main:
                    with st.spinner("Xylia is entering Discovery Mode using the Main Engine — this is deep work..."):
...
End with a one-paragraph RESEARCH VERDICT: your best assessment of which hypothesis is most likely and why, with a confidence level (Low / Medium / High)."""
                        ...
                        try:
                            ...
                            # Deep multimodal research -> Main Tier
...
"""
    ...
    def generate_discovery_pdf(self) -> Optional[bytes]:
        """Generate a colorful PDF specifically for the Frontier Discovery session"""
        try:
            html_content = f"""
            <!DOCTYPE html>
            <html>
            <head>
                ...
                <style>
                    @page {{ size: A4; margin: 1.5cm; }}
                    body {{ font-family: 'Helvetica', 'Arial', sans-serif; background-color: #ffffff; color: #1a1a1a; margin: 0; padding: 0; line-height: 1.5; }}
                    .header {{ background: linear-gradient(135deg, #FF5722 0%, #FF9800 100%); padding: 30px; border-radius: 12px; color: white; text-align: center; margin-bottom: 30px; }}
                    .title {{ font-size: 32px; font-weight: bold; margin: 0; }}
                    .subtitle {{ font-size: 16px; opacity: 0.9; margin-top: 5px; }}
                    .section {{ margin-bottom: 25px; padding: 20px; background: #fdfdff; border-radius: 8px; border-left: 6px solid #FF5722; }}
                    .section-title {{ font-size: 20px; font-weight: bold; color: #FF5722; margin-bottom: 12px; text-transform: uppercase; }}
                    .message {{ margin-bottom: 15px; padding: 15px; border-radius: 8px; }}
                    .nik {{ background-color: #f0f4f8; border-left: 4px solid #607D8B; }}
                    .xylia {{ background-color: #fff3e0; border-left: 4px solid #FF9800; }}
                    .role {{ font-weight: bold; margin-bottom: 5px; color: #333; }}
                    .content {{ white-space: pre-wrap; }}
                    .footer {{ text-align: center; color: #888; font-size: 11px; margin-top: 50px; border-top: 1px solid #eee; padding-top: 20px; }}
                </style>
            </head>
            <body>
                <div class="header">
                    <div class="title">🔭 Xylia | Frontier Discovery</div>
                    <div class="subtitle">Autonomous Research Session</div>
                </div>
                
                <div class="section" style="border-left-color: #00BCD4;">
                    <div class="section-title">Session Profile</div>
                    <p><strong>Generated:</strong> {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                    <p><strong>Messages:</strong> {len(st.session_state.discovery_messages)}</p>
                </div>
                
                <div class="section">
                    <div class="section-title">Research Protocol Log</div>
            """
            
            for msg in st.session_state.discovery_messages:
                role = "Nik" if msg['role'] == 'user' else "Xylia"
                css_class = "nik" if msg['role'] == 'user' else "xylia"
                content_md = markdown.markdown(msg['content'])
                html_content += f"""
                    <div class="message {css_class}">
                        <div class="role">[{role}]</div>
                        <div class="content">{content_md}</div>
                    </div>
                """
            ...
            st.error(f"Discovery PDF Generation Error: {str(e)}")
            return None
    ...
    def render_history_sidebar(self):
        ...
        with st.sidebar:
            ...
            discovery_data = self.db_manager.discovery_db.all() if hasattr(self.db_manager, 'discovery_db') else []
            ...
            if all_data or chat_data:
                if export_format == "JSON":
                    ...
                    full_export = {
                        ...
                        "discovery_sessions": discovery_data,
                        "qa_messages": st.session_state.get('display_messages', []),
                        "qa_sessions": chat_data,
...
"""
                    ...
                    # Stored Q&A Sessions
                    if chat_data:
                        txt += f"\n{'='*60}\n## SAVED Q&A SESSIONS ({len(chat_data)})\n{'='*60}\n"
                        for session in chat_data:
                            txt += f"\n--- {session.get('title', 'Saved Chat')} ---\n"
                            timestamp = session.get('timestamp', 'Unknown')
                            txt += f"Date: {timestamp}\n"
                            for msg in session.get('display_messages', []):
                                role = "Nik" if msg['role'] == 'user' else "Xylia"
                                txt += f"\n[{role}]: {msg['content']}\n"
                    ...
                    disc_msgs = st.session_state.get('discovery_messages', [])
                    if len(disc_msgs) > 1:
                        txt += f"\n{'='*60}\n## ACTIVE FRONTIER DISCOVERY SESSION\n{'='*60}\n"
                        for msg in disc_msgs:
                            role = "Nik" if msg['role'] == 'user' else "Xylia"
                            txt += f"\n[{role}]: {msg['content']}\n"
                    
                    # Stored Discovery Sessions
                    if discovery_data:
                        txt += f"\n{'='*60}\n## SAVED DISCOVERY SESSIONS ({len(discovery_data)})\n{'='*60}\n"
                        for disc in discovery_data:
                            txt += f"\n--- {disc.get('title', 'Saved Discovery')} ---\n"
                            timestamp = disc.get('timestamp', 'Unknown')
                            txt += f"Date: {timestamp}\n"
                            for msg in disc.get('display_messages', []):
                                role = "Nik" if msg['role'] == 'user' else "Xylia"
                                txt += f"\n[{role}]: {msg['content']}\n"
    ...
    def render_discovery_mode(self):
        ...
        if len(st.session_state.discovery_messages) > 1:
            ...
            with f_col:
                ...
                if getattr(self, 'WEASYPRINT_AVAILABLE', False) or globals().get('WEASYPRINT_AVAILABLE', False):
                    format_options.append("PDF")
                exp_fmt = st.selectbox("Export Format", format_options, key="disc_fmt", label_visibility="collapsed")
            
            # Build current session export text for raw formats
            ...
            with b_col:
                if exp_fmt == "TXT":
                    ...
                    pdf_bytes = self.generate_discovery_pdf()
                    if pdf_bytes:
                        st.download_button("📥 Download PDF", data=pdf_bytes, file_name="discovery_session.pdf", mime="application/pdf", key="disc_export_pdf", use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
...
Address Nik gently as your fellow researcher."""
                        ...
                        # Deep multimodal reasoning -> Main Tier
                        response = self.ai_engine.model_main.generate_content(
                            payload,
